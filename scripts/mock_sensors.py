...
def playback_sensor_data(input_file, channel, loop):
    ...
    while True:
        ...
        for line in reader:
            msg = can.Message(arbitration_id=int(line[1]), dlc=None, data=bytes.fromhex(line[2]))
...
def send_sensor_data(device, SID, channel):
    ...
    if device == 'bms':
        ...
        while True:
            # cycles through all of the CAN ID frames for the gparser.add_argument("-d", "--device_id", help="device to simulate. Options: gps, bms, wind, acc", default='all')ps
            for i in range(len(SID[device])):
...
def main():
    ...
    parser.add_argument("-f", "--file_name", help="Input the sensor data file you wish to mock", default=None)
    parser.add_argument("--loop", help="Continuously playback the sensor data in a loop", action='store_true')    
