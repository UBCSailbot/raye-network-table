...
int main() {
    ...
    socket.bind("tcp://*:" + std::to_string(NetworkTable::kCentralControllerPort));
    ...
    while (true) {
        ...
        if (message_parts.size() == 2) {
            std::string action = message_parts[0];
            std::string key = message_parts[1];
            ...
            std::string action = message_parts[0];
            std::string key = message_parts[1];
            std::string value = message_parts[2];
